...
!            outp=RESHAPE ((/3, 4, 5, 6, 7, 8, 9 , 10/), (/2, 4/));
            ...
            do k = 1,numRowBlocksInZone
                do j = 1,numRowBlocksInZone
                    ...
                    !tempA_Matrix( ( (k - 1)*inputParamsData%blockSizeValue + 1 ):( k*inputParamsData%blockSizeValue ), &
...
!                    write(*,*)'Copying record for iteration: k=', k, 'and j=',j, 'Zone ID:',i
                   ...
                   tempA_Matrix(((k-1)*ibs+1):(k*ibs),((j-1)*ibs+1):(j*ibs)) &
                    ...
                    !tempB_Matrix( ( (k - 1)*inputParamsData%blockSizeValue + 1 ):( k*inputParamsData%blockSizeValue ), &
                    !              ( (j - 1)*inputParamsData%blockSizeValue + 1 ):( j*inputParamsData%blockSizeValue ) ) &
                    !            = reshape( recordRow(3:( inputParamsData%blockSizeValue*inputParamsData%blockSizeValue ) + 2), &
                    !              [inputParamsData%blockSizeValue, inputParamsData%blockSizeValue] );
            ...
            index1=3*(size(dataContainer%ZonesData( i )%CoordsNodesInZone( :, 1 ) , 1)- IndepInterfNodesCount);
            
            !┌────────────────────────────────────────┐
            !│FILL A0L SUB-MATRIX FOR THE CURRENT ZONE│
            !└────────────────────────────────────────┘            
            !dataContainer%ZonesData( i )%A0L(:, :) = tempA_Matrix( 1:( 3*size( dataContainer%ZonesData( i )%CoordsNodesInZone( :, 1 ) , 1) - 3*IndepInterfNodesCount ), &
